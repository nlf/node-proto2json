...
case 1:
    ...
    for (ii in this.$) {
      if (this.$.hasOwnProperty(ii)) {
        if (this.$[ii].type === 'package') {
          // Package element
          result.package = this.$[ii].name;
        } else if (this.$[ii].type === 'enum') {
          // Enum element
          _enum = {};
          for (jj in this.$[ii].fields) {
            if (this.$[ii].fields.hasOwnProperty(jj)) {
              _field = this.$[ii].fields[jj];
              _enum[_field.name] = parseInt(_field.tag);
            }
          }
          result.enums[this.$[ii].name] = _enum;
        } else if (this.$[ii].type === 'message') {
          // Message element
          _message = {
            enums: {},
            fields: {}
          ...
          for (jj in this.$[ii].enums) {
            if (this.$[ii].enums.hasOwnProperty(jj)) {
              _enum = this.$[ii].enums[jj];
          ...
          result.messages[this.$[ii].name] = _message;
        }
      }
    }
